...
def main(window):
    ...
    generatorOutputLabel = Label(text='Original message is '+message+'\n Appended part is is ' + appended_part + '\n' +
                                 'Transmitted message is '+ transmitted_message, font='sans-serif 18 bold'
                                 , fg='#DA611E', pady=30
                                 )
    ...
    orLabel=Label(text='or',font='sans-serif 18 bold'
                ,fg='#666666',pady=10
    )
    orLabel.grid(column=0,row=4)
   
    alterButton=Button (text='Alter then verify',bg='#EA5E3D',command=lambda:Alter(transmitted_message),activebackground='#000000',fg='#FFFFFF',activeforeground='#FFFFFF',font="Helvetica 20",width=30,height=1)
    ...
    def output(verification,transmitted_message):
        ...
        if alterState:
            ...
            verifierOutputLabel = Label(text='Altered message is ' + transmitted_message
                                    + ' \n Verifier Output: ' + verification, font='sans-serif 18 bold'
                                     ,fg='red', pady=30)
            alterState = False

        else:
            verifierOutputLabel = Label(text='Verifier Output: ' + verification, font='sans-serif 18 bold'
                                     ,fg='green', pady=30)
    ...
    def takeIndex():
        index=simpledialog.askinteger("index number", prompt='please enter index number to be manipulated')
        return index
